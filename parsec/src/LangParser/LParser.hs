module LangParser.LParser where
...
import qualified LangParser.AstStmt as Ast

type Parser = Parsec Void String

keywords :: [String]
keywords = ["if", "else", "loop", "in", "out"]
...
operators = [
    ...
    [lBinOpParser "+", lBinOpParser "-"],
    [nBinOpParser "==", nBinOpParser "!=", nBinOpParser "<=", nBinOpParser ">=", nBinOpParser "<", nBinOpParser ">" ],
    [rBinOpParser "&&"],
    [rBinOpParser "||"]]
...
ifParser = lexemeSplitter $ do 
    ...
    word "else"
    Ast.If condition ifStatement . Just <$> compoundStatementParser
...
statementParser = lexemeSplitter $ ifParser <|> whileParser <|> assignParser <|> readParser <|> writeParser
